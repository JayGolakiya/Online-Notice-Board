...
namespace NewOnlineNoticeBoard.Controllers
{
    public class NoticeToUserController : Controller
    {
        ...
        public ActionResult UserCanSelect(string err)
        {
            ...
            {
                var cen = db.CE_Notices.Select(s => s).Distinct();
                var itn = db.IT_Notices.Select(s => s).Distinct();
                var ecn = db.EC_Notices.Select(s => s).Distinct();
                var icn = db.IC_Notices.Select(s => s).Distinct();
                var mhn = db.MH_Notices.Select(s => s).Distinct();
                var chn = db.CH_Notices.Select(s => s).Distinct();
                //List<List<CE_Notice>> notices = new List<List<CE_Notice>>();
                //notices.Add(cen);
                //notices.Add(itn);
                ViewData["cen"] = cen;
                ViewData["itn"] = itn;
